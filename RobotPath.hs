...
findPath start end method = do
                            putStrLn $ showRP $ makePath initRobotGrid path badNodesList
...
makePath :: Array (Int,Int) (Maybe Path) -> Branch -> [Node] -> Array (Int,Int) (Maybe Path)
makePath arr path blocks = arr//((map (\x -> (x,Just $ Pt )) path) ++ (map (\x -> (x,Just $ Blk )) blocks))
